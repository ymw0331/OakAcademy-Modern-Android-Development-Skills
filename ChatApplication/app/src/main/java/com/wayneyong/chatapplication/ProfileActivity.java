...
public class ProfileActivity extends AppCompatActivity {
    ...
    public void updateProfile() {
        ...
        if (imageControl) {
            //need to identify diff identity for each image
                        ...
                        public void onSuccess(Uri uri) {
                            ...
                            reference.child("Users").child(auth.getUid()).child("image").setValue(filePath).addOnSuccessListener(new OnSuccessListener<Void>() {
                                ...
                                public void onCanceled() {
                                    Toast.makeText(ProfileActivity.this, "Problem occured", Toast.LENGTH_LONG).show();
                                }
                            });
                        }
                    });
    ...
    public void getUserInfo() {
        ...
        reference.child("Users").child(firebaseUser.getUid()).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                //import username and image add from db to string variables
                String name = snapshot.child("userName").getValue().toString().trim();
                image = snapshot.child("image").getValue().toString().trim();
                editTextUserNameProfile.setText(name);

                if (image.equals("null")) {
                    imageViewCircleProfile.setImageResource(R.drawable.ic_baseline_account_box_24);
                } else {
